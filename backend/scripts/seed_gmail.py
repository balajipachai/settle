...
def mime(subject: str, body: str, when: datetime, msg_id: str, in_reply_to: str | None = None) -> str:
    m = EmailMessage()
    m["From"] = "Maya Chen <maya@acme-analytics.example>"
    m["To"] = inbox_address
    m["Subject"] = subject
...
prior_id = make_msgid(domain="acme-analytics.example")
...
dispute = gmail._req("POST", "/messages", json={
    "raw": mime(f"Duplicate API overage charge on {ref}", ACME_DISPUTE_BODY.replace("INV-10428", ref),
                datetime.now(UTC), make_msgid(domain="acme-analytics.example"), in_reply_to=prior_id),
